...
class Game:
    ...
    def load_characters(self):
        char1_names = list(char_dict_player1)
        char2_names = list(char_dict_player2)
    ...
    def place_char(self, pos):
        if self.selected_char:
            ...
            if cell_pos and not self.selected_char.placed:
                x, y = cell_pos
                if name in char_dict_player1 and (13 <= y < 20):
                    self.selected_char.place(x, y)
                    self.placed_characters.append(self.selected_char)
                elif name in char_dict_player2 and 0 <= y <  7:
                    self.selected_char.place(x, y)
                    self.placed_characters.append(self.selected_char)
    ...
    def attack(self,attacker,victim):
        if attacker not in self.attacked_character and attacker in char_dict_player1:
            damage = char_dict_player1[attacker].damage
            char_dict_player2[victim].HP -= damage

        elif attacker not in self.attacked_character and attacker in char_dict_player2:
            damage = char_dict_player2[attacker].damage
            char_dict_player1[victim].HP -= damage
        ...
        if self.selected_pic:
            ...
            if selected_character:
                ...
                if scaled_image:
                    ...
                    elif self.selected_pic in char_dict_player2 and self.turn == 1:
                        self.highlight_movement_range(self.selected_pic,2,2)
                        self.highlight_attack_range(self.selected_pic) 
                        screen.blit(scaled_image, (75, 50))
                        self.stat_show(100, 400, 200, 200, 20, selected_character)
